	...
	angular.module('HousePointsApp', ['ui.router', 'ngCookies', 'restangular', 'ngAnimate', 'LocalStorageModule', 'ngAria', 'ngMaterial'])
		...
		.config(function ($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider, $animateProvider, $compileProvider) {
			...
			$stateProvider
				...
				.state('kids.add', {
					url: '/add',
					templateUrl: 'main/parentView/AddChild/add-child.html',
				...
				.state('kids.display', {
					...
					controllerAs: 'childDetailsVm',
					templateUrl: 'main/parentView/childDetails/childDetails.html',
					resolve: {
						childDetails: function (KidsService, $q) {
							var defer = $q.defer();
							KidsService.getAll().then(function (data) {
								defer.resolve(data);
							});
